...
\item HTTP service: a Flask app at full parity with
  \texttt{api.tezaurs.lv}. Wraps the engine plus a port of the FSA
  tokenizer (\texttt{Splitting.java}), the paradigm-suitability
  scorer, and short multi-word helpers for phrase and personal-name
  inflection.
\item Test suite: 58 tests, all passing as of submission. Includes 9
  parity tests against the gold corpus, 7 canonical-word acceptance
  tests, 17 tests covering the HTTP layer (tokenizer, phrase inflection,
  suitable-paradigm scorer, endpoint smoke tests).
