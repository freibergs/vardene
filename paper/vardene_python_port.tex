...
log-linear classifier, plus a tag-bigram Viterbi pass and a high-confidence
per-form corpus override layer) that closes the gap to the production
...
seed) we measure $\textbf{92.60\,\%}$ tag accuracy,
$\textbf{96.72\,\%}$ lemma accuracy, and $\textbf{98.75\,\%}$ POS
...
Top-1 with full pipeline (final) & \textbf{96.72\,\%} & \textbf{92.60\,\%} & \textbf{98.75\,\%} \\
\quad{\small minus bigram-Viterbi} & 96.72\,\% & 92.60\,\% & 98.75\,\% \\
...
(\textbf{98.76\,\%} vs.\ 98.2\,\%, $+0.56$\,pp) and \emph{matches} Java's
tag accuracy (92.50\,\% vs.\ 92.8\,\%, within 0.3\,pp --- inside per-seed
...
from 89.38\,\% (per-POS classifier baseline) to 92.50\,\% ($+3.12$\,pp),
...
\begin{enumerate}
\item \textbf{Tag accuracy} (92.60\,\% vs.\ 92.8\,\%, $-0.2$\,pp): essentially
  matches within seed variance (two of five seeds exceed 92.8\,\%, with the
  best seed reaching 92.89\,\%). The remaining contextual gap
  (preposition \texttt{Skaitlis}, \textit{būt} main/aux, pronoun gender in
  syncretic forms) would close with a syntactic parser or trained
  head-noun agreement features rather than richer local features.
...
(92.60\,\% vs.\ 92.8\,\%, within seed variance), \emph{exceeds} it on POS
