...
\texttt{PeterisP/morphology} (the morphology engine itself, $\sim$9.3\,kLOC) and
\texttt{LUMII-AILab/Webservices} \cite{lumii_webservices} (the HTTP service
layer that exposes the engine at \texttt{api.tezaurs.lv}, $\sim$3\,kLOC,
also GPL-3.0). The first ships the algorithmic core
(\texttt{Analyzer.analyze()} returns the candidate readings ranked by an
additive scoring rule \texttt{Word.getBestWordform()}); the second wraps it
in 16 HTTP routes including the multi-word phrase inflectors, personal-name
inflector, valency-tag annotation tool, and paradigm-suitability scorer.

The production tagger that backs \texttt{api.tezaurs.lv}'s
\texttt{/morphotagger} endpoint --- which contributes the published 92.8\,\%
tag accuracy --- is a third, separate project (\texttt{LVTagger}) that
consumes the morphology library's output and re-ranks via a sentence-level
CMM/CRF tagger. We port the morphology engine and the HTTP service layer
in full, and additionally train a Python-native sentence-level
disambiguator from the same gold corpus that LVTagger was trained on, since
\texttt{LVTagger} itself is not part of either Java repository.
...
The closest comparison point to this work is \textbf{LVTagger}, a
sentence-level CMM/CRF tagger \cite{paikens2013} trained on the same
\texttt{train.txt} gold corpus we use here. LVTagger consumes the
output of \texttt{PeterisP/morphology} as its candidate set, which is
the role our Python disambiguator plays. We do not directly replicate
LVTagger's CMM architecture; we use a 4-character subtag CRF plus
per-POS log-linear classifier plus tag-bigram Viterbi rescoring (see
Section~\ref{sec:disamb}). Direct head-to-head accuracy comparison
against UDPipe and Stanza is left to future work because the training
data, tagsets, and evaluation protocols all differ.
...
Throughout the port we maintained a regression suite of 65 tests covering
unit behaviour (Mijas, Trie, Attributes, Paradigm, Inflector,
MarkupConverter), tokenizer parity (Splitting state machine + 1{,}622
lexicon abbreviations), HTTP API smoke tests, multi-word phrase
inflection, valency-tag annotation, and corpus parity
(\texttt{train.txt} from the LVTagger repository).
...
\midrule
\multicolumn{4}{l}{\textit{Held-out 20\,\% split, 5-seed average ($n \approx 3{,}500$/seed; mean $\pm$ std)}}\\
Top-1 with full pipeline (final) & $\mathbf{96.73 \pm 0.29}$ & $\mathbf{92.51 \pm 0.29}$ & $\mathbf{98.75 \pm 0.16}$ \\
\quad\textit{per-seed range} & $[96.26, 97.03]$ & $[92.06, 92.79]$ & $[98.60, 99.00]$ \\
