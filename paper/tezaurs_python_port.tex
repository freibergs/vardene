...
0.55 points. The Python package is 41\,\% smaller in source size, has a
...
\midrule
Mijas (stem alternations) & 3{,}500 & 1{,}960 & 76 mija handlers, two directions \\
Analyzer & 1{,}124 & 1{,}161 & Analysis, prefix stripping, guessing \\
...
Trie/Wordform/Other & 1{,}784 & 219 & Various data classes \\
\midrule
\textbf{Total runtime} & \textbf{9{,}316} & \textbf{5{,}049} & \textbf{Python is 46\,\% smaller} \\
\bottomrule
\end{tabular}
\caption{Source size by component. Counts exclude the disambiguator
(\texttt{crf\_tagger.py}, 469 LOC, an addition that has no Java counterpart in
the morphology repo).}
\label{tab:loc}
\end{table}
...
\midrule
Source LOC (engine) & 9{,}316 & 5{,}049 \\
Source LOC (engine + disambig.) & --- & 5{,}519 \\
...
\item \textbf{Code size} (5{,}049 vs.\ 9{,}316 LOC, $-46$\,\%): no
...
\item Source: data-extraction tools (\texttt{tools/}) and runtime engine
  (\texttt{tezaurs/}) total 5{,}519 LOC plus 800 LOC of pipeline code.
...
46\,\% reduction in source size, an 8$\times$ reduction in shipped reference
